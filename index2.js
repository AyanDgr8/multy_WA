...
app.post('/:instanceId/send-media', upload.single('mediaFile'), async (req, res) => {
    const instanceId = req.params.instanceId;
    ...
    try {
        ...
        const fileExtension = path.extname(req.mediaFile.originalname).toLowerCase();

        console.log('File uploaded at:', req.mediaFile.path);

        // Prepare the message payload
        let messagePayload = { caption };
        const media = await prepareWAMessageMedia({ url: req.mediaFile.path }, { upload: instanceSock.upload });
        ...
        await logMediaMessageToDB(instanceId, phoneNumbers, message, req.file.path, caption, req.body.schedule_time, 'success');

        res.send({
            message: 'Media messages sent successfully!',
            filePath: req.mediaFile.path
        });
    } catch (error) {
